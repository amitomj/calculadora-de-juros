...
const App: React.FC = () => {
  ...
  // Inicializa o tema com prioridade para localStorage e sistema
  useEffect(() => {
    ...
    // Aplicação imediata no DOM para evitar flash branco
  ...
  // Sincroniza o estado do tema com o documento e armazenamento
  useEffect(() => {
    if (!mounted) return;
  ...
  // Previne renderização inconsistente antes da montagem do tema
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-moon-bg text-slate-900 dark:text-slate-100 transition-colors duration-300 pb-10">
      <header className="bg-white dark:bg-moon-card border-b border-slate-200 dark:border-moon-border sticky top-0 z-50">
        <div className="max-w-5xl mx-auto px-4 py-3 flex flex-col sm:flex-row items-center justify-between gap-4">
          <div className="flex items-center gap-3 w-full sm:w-auto">
            ...
            <button 
              onClick={toggleTheme}
              className="sm:hidden p-2 rounded-xl bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-400 border border-slate-200 dark:border-moon-border"
          ...
          <div className="flex items-center gap-2 w-full sm:w-auto">
            <div className="grid grid-cols-2 gap-1 bg-slate-100 dark:bg-slate-800 p-1 rounded-xl w-full sm:w-auto">
              <button 
                ...
                className={`px-3 py-2 md:px-4 rounded-lg text-xs md:text-sm font-semibold transition-all cursor-pointer ${activeTab === 'interest' ? 'bg-white dark:bg-slate-700 text-blue-600 dark:text-blue-300 shadow-sm' : 'text-slate-500 hover:text-slate-800 dark:hover:text-slate-200'}`}
              >
                Juros de Mora
              </button>
              <button 
                onClick={() => setActiveTab('devaluation')}
                className={`px-3 py-2 md:px-4 rounded-lg text-xs md:text-sm font-semibold transition-all cursor-pointer ${activeTab === 'devaluation' ? 'bg-white dark:bg-slate-700 text-blue-600 dark:text-blue-300 shadow-sm' : 'text-slate-500 hover:text-slate-800 dark:hover:text-slate-200'}`}
              >
                Atualização
              </button>
            </div>
            <button 
              onClick={toggleTheme}
              className="hidden sm:flex p-2.5 rounded-xl bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-400 hover:ring-2 hover:ring-blue-500 transition-all border border-slate-200 dark:border-moon-border cursor-pointer"
            >
              {theme === 'day' ? (
                <svg width="20" height="20" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><path d="M21 12.79A9 9 0 1 1 11.21 3 7 7 0 0 0 21 12.79z"></path></svg>
              ) : (
      ...
      <main className="max-w-5xl mx-auto px-4 pt-6 md:pt-10">
        <div className="mb-6 md:mb-10 animate-in fade-in">
          ...
          <p className="text-sm md:text-lg text-slate-500 dark:text-slate-400 max-w-2xl leading-relaxed">
            {activeTab === 'interest' 
              ? 'Calculadora profissional para juros legais civis, comerciais e dívidas ao Estado.' 
              : 'Conversão de valores com base nos coeficientes oficiais de desvalorização (1903-2015).'}
          </p>
        </div>
      ...
      <footer className="max-w-5xl mx-auto px-4 mt-12 pt-6 border-t border-slate-200 dark:border-moon-border text-slate-400 dark:text-slate-500 text-xs flex flex-col sm:flex-row justify-between items-center gap-4">
        <p>&copy; {new Date().getFullYear()} Calculadora Jurídica • Portugal</p>
        <div className="flex gap-4">
          <span>v1.5.0 (AI Studio Optimized)</span>
        </div>
      </footer>
    </div>
  );
